...
public class MoneyList {
	...
	//plug in first.next as n
	public double recuriveSum(MoneyNode n) {
		//base case: empty linked list
		if (n==null)
			return 0;
		if(n.data instanceof Bill)
		
		return ((Bill)n.data).getValue()+recuriveSum(n.next);
		else
			return ((Coin)n.data).getValue()/100.0+recuriveSum(n.next);
			
	}
	
	//plug in first.next as n
		public double recuriveSumQuarter(MoneyNode n) {
			//base case: empty linked list
			if (n==null)
				return 0;
			if(n.data instanceof Quarter)
			
			return ((Quarter)n.data).getValue()/100.0+recuriveSumQuarter(n.next);
			else //not a quarter, skip it. 
				return recuriveSumQuarter(n.next);
